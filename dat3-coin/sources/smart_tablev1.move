...
/// SmartTablev1 uses faster hash function SipHash instead of cryptographically secure hash functions like sha3-256 since
/// it tolerates collisions.
module dat3::smart_tablev1 {
    use std::error;
    use std::vector;
    ...
    /// SmartTablev1 entry contains both the key and value.
    ...
    struct SmartTablev1<K, V> has store {
    ...
    public fun new<K: copy + drop + store, V: store>(): SmartTablev1<K, V> {
        new_with_config<K, V>(0, 0, 0)
    }

    /// Create an empty SmartTablev1 with customized configurations.
    ...
    public fun new_with_config<K: copy + drop + store, V: store>(
        ...
        let table = SmartTablev1 {
            ...
            target_bucket_size,
            keys: smart_vector::empty_with_config<K>(10000000, 200)
        };
    ...
    public fun destroy_empty<K, V>(table: SmartTablev1<K, V>) {
        ...
        let SmartTablev1 {
            buckets, num_buckets: _, level: _, size: _,
            split_load_threshold: _, target_bucket_size: _, keys: _keys
        } = table;
        smart_vector::destroy_empty(_keys);
        table_with_length::destroy_empty(buckets)
    }

    /// Destroy a table completely when V has `drop`.
    public fun destroy<K: drop, V: drop>(table: SmartTablev1<K, V>) {
        let i = 0;
        while (i < table.num_buckets) {
    ...
    public fun add<K: copy + drop + store, V>(table: &mut SmartTablev1<K, V>, key: K, value: V) {
        ...
        smart_vector::push_back(&mut table.keys, e.key);
    ...
    fun split_one_bucket<K, V>(table: &mut SmartTablev1<K, V>) {
    ...
    public fun borrow<K: drop, V>(table: &SmartTablev1<K, V>, key: K): &V {
    ...
    public fun borrow_with_default<K: copy + drop, V>(table: &SmartTablev1<K, V>, key: K, default: &V): &V {
    ...
    /// Aborts if there is no entry for `key`.
    public fun borrow_mut<K: drop, V>(table: &mut SmartTablev1<K, V>, key: K): &mut V {
        let index = bucket_index(table.level, table.num_buckets, sip_hash_from_value(&key));
        let bucket = table_with_length::borrow_mut(&mut table.buckets, index);
        let i = 0;
        let len = vector::length(bucket);
    ...
    public fun borrow_mut_with_default<K: copy + drop + store, V: drop>(
        table: &mut SmartTablev1<K, V>,
        key: K,
        default: V
    ): &mut V {
        if (!contains(table, copy key)) {
            smart_vector::push_back(&mut table.keys, key);
    ...
    public fun contains<K: drop, V>(table: &SmartTablev1<K, V>, key: K): bool {
        let hash = sip_hash_from_value(&key);
        let index = bucket_index(table.level, table.num_buckets, hash);
        let bucket = table_with_length::borrow(&table.buckets, index);
    ...
    /// Aborts if there is no entry for `key`.
    public fun remove<K: copy + drop, V>(table: &mut SmartTablev1<K, V>, key: K): V {
        let index = bucket_index(table.level, table.num_buckets, sip_hash_from_value(&key));
        let bucket = table_with_length::borrow_mut(&mut table.buckets, index);
        let i = 0;
        let len = vector::length(bucket);
        while (i < len) {
            let entry = vector::borrow(bucket, i);
            if (&entry.key == &key) {
                let (is, key_index) = smart_vector::index_of(&mut table.keys, &key);
                if (is) {
                    smart_vector::swap_remove(&mut table.keys, key_index) ;
                };
    ...
    public fun upsert<K: copy + drop + store, V: drop>(table: &mut SmartTablev1<K, V>, key: K, value: V) {
        if (!contains(table, copy key)) {
    ...
    public fun length<K, V>(table: &SmartTablev1<K, V>): u64 {
        table.size
    }

    /// Return the load factor of the hashtable.
    public fun load_factor<K, V>(table: &SmartTablev1<K, V>): u64 {
    ...
    public fun update_target_bucket_size<K, V>(table: &mut SmartTablev1<K, V>, target_bucket_size: u64) {
   ...
   public fun smart_table_test() {
    ...
    fun smart_table_split_test() {
        let table: SmartTablev1<u64, u64> = new_with_config(1, 100, 1);
        let i = 1u64;
        let level = 0;
        while (i <= 256) {
